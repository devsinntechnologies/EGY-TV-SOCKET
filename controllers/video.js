...
function transcodeVideo(inputFilePath, outputFilePath, videoName) {
    ffmpeg(inputFilePath)
      ...
      .on('end', async() => {
        ...
        files.sort((a, b) => {
          const numA = parseInt(a.match(/\d+/)[0]); // Extract the numerical counter from filename
          const numB = parseInt(b.match(/\d+/)[0]); // Extract the numerical counter from filename
          return numA - numB;
        });
